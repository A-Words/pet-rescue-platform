...
![业务流程图](images/fig1-2business_flow_diagram.drawio.png)
...
![功能模块结构图](images/fig1-3-functional-module-structure.drawio.png)
...
![顶层数据流图](images/fig1-4-top-level-dfd.drawio.png)
...
![走失宠物实体属性图](images/fig2-2_lost_pet.drawio.png)

图2.2 走失宠物实体属性图

（3）发现线索实体包含线索编号、关联走失宠物编号、上报用户编号、照片地址、描述信息、发现地点、纬度、经度、发现日期、联系方式、状态、管理员备注、审核人编号、审核时间、创建时间等属性。发现线索实体属性图如图2.3所示。

![发现线索实体属性图](images/fig2-3-clue-found.drawio.png)

图2.3 发现线索实体属性图

（4）可领养宠物实体包含宠物编号、宠物名称、宠物种类、品种、颜色、性别、月龄、照片地址、描述信息、健康状况、是否已接种疫苗、是否已驱虫、是否已绝育、领养状态、救助站、入站日期、创建时间、更新时间等属性。可领养宠物实体属性图如图2.4所示。

![可领养宠物实体属性图](images/fig2-4-adoptable-pet.drawio.png)

图2.4 可领养宠物实体属性图

（5）领养申请实体包含申请编号、可领养宠物编号、申请人编号、申请人姓名、申请人电话、申请人地址、申请人身份证号、住房类型、是否饲养其他宠物、领养原因、养宠经验描述、状态、创建时间、更新时间等属性。领养申请实体属性图如图2.5所示。

![领养申请实体属性图](images/fig2-5-adoption-application.drawio.png)

图2.5 领养申请实体属性图

（6）审核记录实体包含记录编号、申请编号、审核人编号、审核决定、审核备注、审核时间等属性。审核记录实体属性图如图2.6所示。

![审核记录实体属性图](images/fig2-6-review-record.drawio.png)

图2.6 审核记录实体属性图

（7）回访提醒实体包含提醒编号、申请编号、领养人编号、可领养宠物编号、提醒日期、回访日期、状态、回访备注、创建时间等属性。回访提醒实体属性图如图2.7所示。

![回访提醒实体属性图](images/fig2-7-followup-reminder.drawio.png)
...
走失宠物管理模块的局部E-R图如图2.8所示。

![走失宠物管理模块局部E-R图](images/fig2-8-lost-pet-manage-er.drawio.png)

图2.8 走失宠物管理模块局部E-R图

领养管理模块的局部E-R图如图2.9所示。

![领养管理模块局部E-R图](images/fig2-9-adoption-manage-er.drawio.png)

图2.9 领养管理模块局部E-R图

### 2.4 绘制全局E-R图

将上述局部E-R图进行整合，消除可能存在的命名冲突和属性冲突，得到全局E-R图如图2.10所示。

![全局E-R图](images/fig2-10-global-er.drawio.png)
...
根据E-R图到关系模型的转换规则，设计初始关系模式如下。其中，主键使用下划线标识，外键使用波浪下划线标识。

（1）用户（<u>用户编号</u>，用户名，邮箱，密码哈希值，手机号，头像地址，角色，是否激活，创建时间，更新时间）

（2）走失宠物（<u>宠物编号</u>，<span style="text-decoration: underline wavy;">发布用户编号</span>，宠物名称，宠物种类，品种，颜色，性别，年龄描述，照片地址，描述信息，走失日期，走失地点，救助站，纬度，经度，联系方式，悬赏金额，状态，创建时间，更新时间）

（3）发现线索（<u>线索编号</u>，<span style="text-decoration: underline wavy;">关联宠物编号</span>，<span style="text-decoration: underline wavy;">上报用户编号</span>，照片地址，描述信息，发现地点，纬度，经度，发现日期，联系方式，状态，管理员备注，<span style="text-decoration: underline wavy;">审核人编号</span>，审核时间，创建时间）

（4）可领养宠物（<u>宠物编号</u>，宠物名称，宠物种类，品种，颜色，性别，月龄，照片地址，描述信息，健康状况，是否已接种疫苗，是否已驱虫，是否已绝育，领养状态，救助站，入站日期，创建时间，更新时间）

（5）领养申请（<u>申请编号</u>，<span style="text-decoration: underline wavy;">宠物编号</span>，<span style="text-decoration: underline wavy;">申请人编号</span>，申请人姓名，申请人电话，申请人地址，申请人身份证号，住房类型，是否饲养其他宠物，领养原因，养宠经验描述，状态，创建时间，更新时间）

（6）审核记录（<u>记录编号</u>，<span style="text-decoration: underline wavy;">申请编号</span>，<span style="text-decoration: underline wavy;">审核人编号</span>，审核决定，审核备注，审核时间）

（7）回访提醒（<u>提醒编号</u>，<span style="text-decoration: underline wavy;">申请编号</span>，<span style="text-decoration: underline wavy;">领养人编号</span>，<span style="text-decoration: underline wavy;">宠物编号</span>，提醒日期，回访日期，状态，回访备注，创建时间）

### 3.3 关系模式优化

对初始关系模式进行函数依赖和规范化检查后可知，各关系模式中的非主属性均完全依赖于对应主键，不存在部分依赖和传递依赖，整体满足第三范式要求。因此，本系统不需要继续进行关系拆分或关系合并。优化工作主要体现在明确主外键约束、唯一约束和业务约束，使关系模式在保持原有结构的基础上更加完整。

优化后的关系模式如下。其中，主键使用下划线标识，外键使用波浪下划线标识。

（1）用户（<u>用户编号</u>，用户名，邮箱，密码哈希值，手机号，头像地址，角色，是否激活，创建时间，更新时间）

该关系模式以用户编号为主键，用户名、邮箱、手机号等非主属性均直接依赖于用户编号，不存在传递依赖，满足3NF。

（2）走失宠物（<u>宠物编号</u>，<span style="text-decoration: underline wavy;">发布用户编号</span>，宠物名称，宠物种类，品种，颜色，性别，年龄描述，照片地址，描述信息，走失日期，走失地点，救助站，纬度，经度，联系方式，悬赏金额，状态，创建时间，更新时间）

该关系模式以宠物编号为主键，发布用户编号作为外键引用用户关系模式。宠物名称、走失地点、状态等非主属性均直接依赖于宠物编号，不存在传递依赖，满足3NF。

（3）发现线索（<u>线索编号</u>，<span style="text-decoration: underline wavy;">关联宠物编号</span>，<span style="text-decoration: underline wavy;">上报用户编号</span>，照片地址，描述信息，发现地点，纬度，经度，发现日期，联系方式，状态，管理员备注，<span style="text-decoration: underline wavy;">审核人编号</span>，审核时间，创建时间）

该关系模式以线索编号为主键，关联宠物编号、上报用户编号和审核人编号作为外键分别关联走失宠物和用户关系模式。线索描述、发现地点、审核状态等非主属性均直接依赖于线索编号，满足3NF。

（4）可领养宠物（<u>宠物编号</u>，宠物名称，宠物种类，品种，颜色，性别，月龄，照片地址，描述信息，健康状况，是否已接种疫苗，是否已驱虫，是否已绝育，领养状态，救助站，入站日期，创建时间，更新时间）

该关系模式以宠物编号为主键，宠物基本信息、健康状况和领养状态等非主属性均直接依赖于宠物编号，不存在传递依赖，满足3NF。

（5）领养申请（<u>申请编号</u>，<span style="text-decoration: underline wavy;">宠物编号</span>，<span style="text-decoration: underline wavy;">申请人编号</span>，申请人姓名，申请人电话，申请人地址，申请人身份证号，住房类型，是否饲养其他宠物，领养原因，养宠经验描述，状态，创建时间，更新时间）

该关系模式以申请编号为主键，宠物编号和申请人编号作为外键分别引用可领养宠物和用户关系模式。申请人信息、住房类型、领养原因、申请状态等非主属性均直接依赖于申请编号。优化时增加（宠物编号，申请人编号）唯一约束，并通过触发器检查宠物领养状态和重复申请情况，从而保证同一用户对同一宠物最多保留一条有效申请记录，满足3NF。

（6）审核记录（<u>记录编号</u>，<span style="text-decoration: underline wavy;">申请编号</span>，<span style="text-decoration: underline wavy;">审核人编号</span>，审核决定，审核备注，审核时间）

该关系模式以记录编号为主键，申请编号和审核人编号作为外键分别引用领养申请和用户关系模式。审核决定、审核备注和审核时间均直接依赖于记录编号，满足3NF。

（7）回访提醒（<u>提醒编号</u>，<span style="text-decoration: underline wavy;">申请编号</span>，<span style="text-decoration: underline wavy;">领养人编号</span>，<span style="text-decoration: underline wavy;">宠物编号</span>，提醒日期，回访日期，状态，回访备注，创建时间）

该关系模式以提醒编号为主键，申请编号、领养人编号和宠物编号作为外键分别引用领养申请、用户和可领养宠物关系模式。提醒日期、回访日期、状态和回访备注等非主属性均直接依赖于提醒编号，满足3NF。

综上，优化后的关系模式与初始关系模式在属性组成上保持一致，但进一步明确了主键、外键、唯一约束和触发器约束，能够减少数据冗余，避免插入异常、删除异常和更新异常。
...
走失宠物模块首先展示走失宠物列表页面，用户可以按照宠物名称、品种、宠物类型和状态进行筛选查询，也可以点击“发布走失信息”按钮进入走失宠物发布页面。走失宠物列表页面如图7.1所示。

![图7.1 走失宠物列表页面](images/fig7-1-lost-pet-list.png)
...
![图7.2 走失宠物发布页面](images/fig7-2-lost-pet-publish.png)
...
领养申请功能首先通过领养中心页面展示符合领养条件的宠物信息，用户可以根据宠物名称、品种和宠物类型进行筛选查询，并从列表中选择心仪宠物进入领养申请页面。领养中心页面如图7.3所示。

![图7.3 领养中心页面](images/fig7-3-adoption-center.png)
...
![图7.4 领养申请页面](images/fig7-4-adoption-application.png)
...
通过本次数据库系统课程设计，完成了宠物走失与领养救助平台数据库系统的完整设计与实现。在需求分析阶段，通过调研宠物救助领域的实际需求，明确了系统的两类用户角色（普通用户和管理员），梳理了走失宠物发布、线索上报、领养申请、审核管理、回访提醒和统计分析等核心业务流程，并绘制了系统边界图、业务流程图、功能模块结构图和数据流图。在概念结构设计阶段，识别了用户、走失宠物、发现线索、可领养宠物、领养申请、审核记录和回访提醒7个实体，分析了各实体之间的联系，绘制了局部E-R图和全局E-R图；在逻辑结构设计阶段，将E-R图转换为7个关系模式，并使用规范化理论验证所有关系模式均满足第三范式要求。

在物理结构设计阶段，系统选择PostgreSQL 16作为数据库管理系统，设计了7张数据表的物理结构，创建了14个索引以优化查询性能。在数据库实施阶段，编写了完整的建表SQL语句，创建了1个视图（v_adoptable_pets）用于简化可领养宠物查询，创建了1个存储过程（sp_monthly_statistics）用于月度统计，创建了4个触发器函数和7个触发器用于实现自动时间戳更新、领养审批级联处理、申请重复检查和回访提醒自动生成等业务逻辑。在系统实现阶段，使用FastAPI框架实现了后端RESTful API，使用Vue 3 + TypeScript + Element Plus实现了前端界面，并使用Docker Compose实现了容器化部署。

在设计过程中，也遇到了一些需要权衡的问题。对于业务逻辑放在数据库层还是应用层的问题，最终选择将核心业务约束（如领养申请重复检查、审批级联操作）通过数据库触发器实现，确保数据一致性不受应用层代码影响。对于多值属性存储问题，系统使用PostgreSQL的数组类型存储照片地址列表，避免了额外的关联表设计。对于统计查询性能问题，则通过创建存储过程将复杂的统计计算放在数据库层执行，减少了网络传输开销。

本系统目前仍存在一些可以继续完善的方向。当前系统缺少数据备份的自动化脚本，后续可添加定时备份任务；地理位置信息仅支持简单的经纬度存储，尚未使用PostGIS进行空间查询优化；系统也缺少全文搜索功能，后续可引入PostgreSQL的全文检索能力提升搜索体验。此外，还可以考虑添加消息通知功能，在领养申请状态变更时通过邮件或短信通知用户。
