...
系统包括走失宠物信息管理（种类、特征、走失地点、联系人）、发现者上报管理、领养宠物信息管理（健康状况、疫苗状态）、领养申请管理、领养审核记录等模块。

数据库对象要求：

- **视图**：查询当前所有待领养宠物（已驱虫/已疫苗）的详细信息及所在救助站。
- **存储过程**：统计某救助站本月的走失上报数量、成功匹配找回数量及宠物领养成功率。
- **触发器**：当领养申请审批通过后，自动更新宠物状态为"已领养"，并禁止其他人再对该宠物提交申请；同时自动生成回访提醒记录（30天后）。
...
- 申请审核：管理员审核领养申请，审核通过后自动更新宠物领养状态、拒绝同宠物的其他待处理申请，并阻止后续申请。
- 回访提醒：领养申请通过后自动生成 30 天回访提醒。
...
- 函数 `sp_monthly_statistics(rescue_station, year, month)`：按救助站统计月度走失上报、找回、领养成功率和线索数据；未传年月时默认统计本月。
- 触发器：自动更新时间戳、处理领养通过后的状态联动、阻止重复或已领养宠物申请、生成 30 天回访提醒。
